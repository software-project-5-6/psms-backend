-- Migration Script: Change ID columns from BIGINT to VARCHAR(4) - MySQL Compatible
-- WARNING: This will delete all existing data. Backup your database first!

-- Step 1: Drop existing tables (this automatically drops foreign keys)
DROP TABLE IF EXISTS project_user_roles;
DROP TABLE IF EXISTS project_invitations;
DROP TABLE IF EXISTS projects;
DROP TABLE IF EXISTS users;

-- Step 2: Recreate users table with VARCHAR(4) ID
CREATE TABLE users (
...
-- Step 3: Recreate projects table with VARCHAR(4) ID
...
-- Step 4: Recreate project_user_roles table with VARCHAR(4) foreign keys
...
-- Step 5: Recreate project_invitations table with VARCHAR(4) foreign keys
